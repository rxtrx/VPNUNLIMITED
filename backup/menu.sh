...
###########- COLOR CODE -##############
colornow=$(cat /etc/serigala/theme/color.conf)
export NC="\e[0m"
export YELLOW='\033[0;33m';
export RED="\033[0;31m" 
export COLOR1="$(cat /etc/serigala/theme/$colornow | grep -w "TEXT" | cut -d: -f2|sed 's/ //g')"
export COLBG1="$(cat /etc/serigala/theme/$colornow | grep -w "BG" | cut -d: -f2|sed 's/ //g')"                    
###########- END COLOR CODE -##########
...
Isadmin=$(curl -sS https://raw.githubusercontent.com/rxtrx/VPNUNLIMITED/permission/main/ipmini  | grep $MYIP | awk '{print $5}')
...
Exp=$(curl -sS https://raw.githubusercontent.com/rxtrx/VPNUNLIMITED/permission/main/ipmini  | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
echo "IP=$host" > /var/lib/serigala-pro/ipvps.conf
echo ""
...
sleep 2
wget -q -O /root/install_up.sh "https://raw.githubusercontent.com/rxtrx/VPNUNLIMITED/update/main/install_up.sh" && chmod +x /root/install_up.sh
sleep 2
./install_up.sh
sleep 5
rm /root/install_up.sh
rm /opt/.ver
version_up=$( curl -sS https://raw.githubusercontent.com/rxtrx/VPNUNLIMITED/update/main/version_up)
echo "$version_up" > /opt/.ver
echo -e "$COLOR1│${NC}  $COLOR1[INFO]${NC} Successfully Up To Date!"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • Serigala V3 •                $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo ""
read -n 1 -s -r -p "  Press any key to back on menu"
menu
...
serverV=$( curl -sS https://raw.githubusercontent.com/rxtrx/VPNUNLIMITED/update/main/version_up)
if [ "$Isadmin" = "ON" ]; then
